...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const searchData = (indexData, queryData) => {
			console.log('searchData', indexData, queryData);
					...
					.then((res) => {
						...
						nextTick(() => {
							...
							nextTick(() => {
								...
								const tempData = res.data.aggregations.histogram.buckets.map(
									(bucket) => {
										return [
											date.formatDate(bucket.key, chartKeyFormat.value),
