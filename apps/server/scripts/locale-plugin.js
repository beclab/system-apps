	...
	files.forEach((file) => {
		...
		Object.keys(fileObj).forEach((key) => {
			if (allKeyArr.indexOf(key) > -1) {
				console.log(
					lang,
					'The repeated UI entry in the language environment is:',
					key
				);
			} else {
				allKeyArr.push(key);
...
const isExistFilesInEN = () => {
	...
	langArr.forEach((lang) => {
		...
		files.forEach((file) => {
			...
			if (isExist < 0) {
				console.log(
					chalk`{red.bold.italic [${lang}]} {The files in the yellowBright folder that are not synchronized with en are:} {yellowBright.bold.underline ${file}}`
				);
			}
		});
	});
